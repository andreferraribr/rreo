...
plot_rolling = function(df){
  ...
  geom_point(data = df%>% filter (mes %in% c(max(mes),max(mes)-years(1))), aes(x=as.Date(mes), y=acumulado),size =5)+
   ...
   geom_label_repel (size = 7, data = df%>% filter (mes %in% c(max(mes), max(mes)-years(1))), aes(x=as.Date(mes), y=acumulado,label=paste0( prettyNum(round(acumulado,1), big.mark = ".", decimal.mark = ",", nsmall = 1), " (",month(mes,label = TRUE) ,")")))+
