...
plot_rolling = function(df){
  ...
  # plotar o point da UF selecionada
    ...
    # 
    geom_label_repel(size = 10, data = df%>% filter (mes %in% c(max(mes),"2020-03-01", max(mes)-years(1))), aes(x=as.Date(mes), y=acumulado,label=paste0( prettyNum(round(acumulado,0), big.mark = ".", decimal.mark = ","), " (",month(mes,label = TRUE) ,")")))+
 geom_point(data = df%>% filter (mes %in% c(max(mes),"2020-03-01",max(mes)-years(1))), aes(x=as.Date(mes), y=acumulado))+
 
  ggtitle(paste0("posição: ",(month(max(as.Date(df$mes)), label=TRUE)), "/",(year(max(as.Date(df$mes)))) , "  em Bi R$"))+
    theme_classic() +
    ylab("R$ bilhões") +
    xlab("") +
    ...
    scale_x_date(limits = c(as.Date ("2008-01-01"),as.Date(max(df$mes))) ,breaks=date_breaks("1 year"),
...
## Receita Corrente Líquida em Bilhoes
...
ggplot(seguridade_total) + 
 ...
 # geom_point(data = rp%>% filter (mês %in% c(max(mês))), aes(x=as.Date(mês), y=movimento))+
  ...
  geom_label_repel(data = seguridade_total %>% filter (mês %in% c(max(mês))), aes(x=as.Date(mês), y=resultado,label=paste0( prettyNum(round(resultado,1), big.mark = ".", decimal.mark = ",") )), size = 7)+
