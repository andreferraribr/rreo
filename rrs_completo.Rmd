...
ggplot(rcl , aes(x = (as.Date(item)),y = acumulado)) + 
  ...
  # plotar o point da UF selecionada
    geom_text(data = rcl%>% filter (item == max(item)), aes(x=as.Date(item), y=acumulado+10,label=paste("R$", round(acumulado,0), "bi")))+
  
  ggtitle(paste0("Receita Corrente Líquida (posição: ",(month(max(as.Date(rcl$item)))), "/",(year(max(as.Date(rcl$item)))),")" ))+
    theme_classic() +
    ylab("R$ bilhões") +
    xlab("") +
    ...
    scale_x_date(limits = c(as.Date ("2010-01-01"),as.Date("2021-09-01")) ,breaks=date_breaks("1 year"),
